...
type partNumber = {value: number, places: number[][]}
...
export async function day3a(dataPath?: string) {
  ...
  const partNumbers: partNumber[] = numbers.filter(n => checkNumber(n, symbols, numRows, numCols))

  return partNumbers.map(n => n.value).reduce((curr, acc) => acc + curr);
}
...
function findNumbers(array: string[][]) {
